                ...
                try {
                    ...
                    if (response.functionCalls && response.functionCalls.length > 0) {
                        ...
                        for (const toolCall of response.functionCalls) {
                            ...
                            if (functionName in available_functions) {
                                try {
                                    ...
                                    if (functionName === 'fetch_website_text') result = await available_functions[functionName](args.url);
                                    else if (functionName === 'list_files') result = available_functions[functionName](args.directory, args.grep);
                                    else if (functionName === 'read_image_file') result = available_functions[functionName](args.image_paths);
                                    else if (functionName === 'create_file') result = available_functions[functionName](args.name, args.extension, args.content);
                                    else if (functionName === 'create_text_file') result = available_functions[functionName](args.file_path, args.content);
                                    else if (functionName === 'get_video_screenshot') result = available_functions[functionName](args.video_path, args.timestamp);
                                    else if (functionName === 'get_target_info') result = available_functions[functionName](args.target_path);
                                    else if (functionName === 'read_text_files') result = available_functions[functionName](args.file_paths, args.read_by_chunks_of_40);
                                    else if (functionName === 'edit_text_files') result = available_functions[functionName](args.file_path, args.chunks);
                            ...
                            if (functionName === 'read_image_file' && String(result).includes("Success")) {
                                let imagePaths = args.image_paths || args.image_path || [];
                                if (typeof imagePaths === 'string') imagePaths = [imagePaths];
                                
                                for (const p of imagePaths) {
                                    const absPath = path.resolve(p);
                                    if (String(result).includes(`Success: Loaded image from '${absPath}'`)) {
                                        try {
                                            ...
                                            console.log(`Failed to load image bytes for ${absPath}: ${e.message}`);
                                        }
                                    }
                                }
                            }
                        ...
                        if (imagesToAttach.length > 0) {
                            contents.push({
                                role: 'user',
                                parts: [...imagesToAttach, { text: "Here are the images you requested to read." }]
                            });
                        }
                    } else {
                        break;
